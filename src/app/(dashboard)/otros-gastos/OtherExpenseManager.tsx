...
export default function OtherExpenseManager({ initialExpenses, role }: { initialExpenses: any[], role: string }) {
    ...
    const filteredExpenses = initialExpenses.filter(expense => {
        ...
        if (searchTerm) {
            ...
            const productMatch = (expense.productOrService || '').toLowerCase().includes(term);
            const providerMatch = (expense.provider || '').toLowerCase().includes(term);
            if (!productMatch && !providerMatch) return false;
        }
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <form action={(formData) => {
                        ...
                        if (!targetId) return;
                        handleAction(
                            () => uploadOtherExpenseReceipt(targetId, formData),
                ...
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar por producto, servicio o proveedor..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="pl-9 pr-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-sm outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 w-64"
                            />
                        </div>
                    </div>
                    ...
                    {activeTab === 'HISTORY' && (
                        <div className="flex flex-wrap gap-4 items-center animate-in fade-in slide-in-from-top-2">
                            <div className="flex items-center gap-2">
                                ...
                                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="rounded-lg border-slate-300 dark:border-slate-700 border p-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-150" />
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Hasta</span>
                                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="rounded-lg border-slate-300 dark:border-slate-700 border p-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-150" />
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        ...
                        <tbody className={isPending ? 'opacity-50 pointer-events-none' : ''}>
                            {filteredExpenses.length === 0 ? (
                                <tr><td colSpan={role === 'ADMIN' ? 9 : 8} className="p-8 text-center text-slate-500 dark:text-slate-400">No hay otros gastos registrados.</td></tr>
                            ...
                            <tfoot>
                                <tr className="bg-slate-50 dark:bg-slate-800/80 border-t-2 border-slate-200 dark:border-slate-700">
                                    ...
                                    <td className="p-3 text-right font-black text-indigo-700 dark:text-indigo-400 text-lg">
                                        {formatCurrency(filteredExpenses.reduce((acc, curr) => acc + curr.totalValue, 0))}
                                    </td>
                                    {role === 'ADMIN' && <td></td>}
                                </tr>
                            </tfoot>
